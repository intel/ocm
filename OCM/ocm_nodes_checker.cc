...
FrameworkNodesChecker::FrameworkNodesChecker(Framework_Names fw,
  ...
  case Framework_Names::ONNX:
    ocmFrameworkObj =
        std::unique_ptr<ONNXRTNodesChecker>(new ONNXRTNodesChecker);
    break;
  default:
  ...
  if ((device_id != "CPU") && (device_id != "GPU") && (device_id != "MYRIAD") &&
      (device_id != "HDDL")) {
    OCM_LOG(3) << "Invalid Device - " << device_id
               << ". Allowed options are CPU, GPU, MYRIAD or HDDL" << std::endl;
  ...
  ocmFrameworkObj->device_id = device_id;
   if ((ov_version != "2021.1") && (ov_version != "2021.2") &&
      ...
      (ov_version != "2022.1.0") && (ov_version != "2022.2.0") &&
      (ov_version != "2022.3.0")) {
    OCM_LOG(3) << "Invalid OpenVINO version - " << ov_version
               << ". Allowed options are 2021.1, 2021.2, 2021.3, 2021.4, 2022.1.0, 2022.2.0, and 2022.3.0" << std::endl;
