...
then
  ...
  cd ${TF_SRC_DIR}/tensorflow; git checkout v2.6.0; 
else
	cd ${TF_SRC_DIR}
  git clone https://github.com/tensorflow/tensorflow.git
  echo "Checking out the required version"
  cd tensorflow; git checkout v2.6.0; 
fi 
...
pip3 install --force-reinstall /tmp/tensorflow_pkg/tensorflow-2.6.0*.whl

echo "Copying the tensorflow wheel to ${TF_SRC_DIR}/tensorflow/"
cp /tmp/tensorflow_pkg/tensorflow-2.6.0*.whl ${TF_SRC_DIR}/tensorflow/
