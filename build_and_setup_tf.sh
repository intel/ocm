...
then
  ...
  cd ${TF_SRC_DIR}/tensorflow; git checkout v2.5.0; 
else
	cd ${TF_SRC_DIR}
...
pip3 install --force-reinstall /tmp/tensorflow_pkg/tensorflow-2.5.0*.whl

echo "Copying the tensorflow wheel to ${TF_SRC_DIR}/tensorflow/"
cp /tmp/tensorflow_pkg/tensorflow-2.5.0*.whl ${TF_SRC_DIR}/tensorflow/
