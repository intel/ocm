...
then
  ...
  cd ${TF_SRC_DIR}/tensorflow; git checkout v2.5.1; 
else
	cd ${TF_SRC_DIR}
  git clone https://github.com/tensorflow/tensorflow.git
  echo "Checking out the required version"
  cd tensorflow; git checkout v2.5.1; 
fi 
...
pip3 install --force-reinstall /tmp/tensorflow_pkg/tensorflow-2.5.1*.whl

echo "Copying the tensorflow wheel to ${TF_SRC_DIR}/tensorflow/"
cp /tmp/tensorflow_pkg/tensorflow-2.5.1*.whl ${TF_SRC_DIR}/tensorflow/
